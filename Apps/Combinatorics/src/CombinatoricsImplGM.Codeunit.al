codeunit 70003 "Combinatorics Impl. GM"
{
    ...
    var
        IncorrectVariationErr: Label 'The value of k must be less than or equal to the value of n, and both numbers must be greater than or equal to 0.';
    begin
        if (n < 0) or (k < 0) or (n < k) then
            Error(IncorrectVariationErr);

        exit(Factorial(n) / Factorial(n - k));
    end;

    procedure VariationWithRepetition(n: Integer; k: Integer): BigInteger
    var
        Math: Codeunit Math;
        IncorrectVariationWithRepetitionErr: Label 'Both numbers must be greater than or equal to 0.';
    begin
        if (n < 0) or (k < 0) then
            Error(IncorrectVariationWithRepetitionErr);
